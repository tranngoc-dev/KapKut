...
DEFAULT_DEVICE = {
    ...
    "os_version": "15.7.4",
    "device_id": "7864123456789012345",
    "iid": "7865123456789012345",
    "region": "VN",
    "loc": "VN",
    "lan": "vi-VN",
    "pf": "3",
    "tdid": "7866123456789012345",
}
...
def tts_new_body(texts, voice, resource_id, rate, device, pitch=0):
    ...
    pitch_str = f"{int(pitch):+d}" if int(pitch) != 0 else "0"
    voice_blocks = []
    for text in texts:
        voice_blocks.append(
            ...
            f'        <prosody rate="{rate}" pitch="{pitch_str}">{escape_xml(text)}</prosody>\n'
            f'    </voice>'
        )
    ssml = (
...
def build_request(args):
    ...
    if args.mode == "tts-new":
        ...
        babi, body = tts_new_body(texts, args.voice, args.resource_id, args.rate, device, pitch=pitch)
